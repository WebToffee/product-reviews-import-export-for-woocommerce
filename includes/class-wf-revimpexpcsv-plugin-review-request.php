...
class Product_review_import_export_Review_Request
{
    /**
     * config options 
     */
    private $plugin_title               =   "Product Reviews Import Export for WooCommerce";
    private $review_url                 =   "https://wordpress.org/support/plugin/product-reviews-import-export-for-woocommerce/reviews/#new-post";  
    private $plugin_prefix              =   "wtpripre"; /* must be unique name */
    private $activation_hook            =   "wtpripre_activate"; /* hook for activation, to store activated date */
    private $deactivation_hook          =   "wtpripre_deactivate"; /* hook for deactivation, to delete activated date */
    ...
    public function __construct()
    {
        ...
        if ($this->check_condition()) /* checks the banner is active now */ {
            $this->banner_message = sprintf(__("Hey, we at %sWebToffee%s would like to thank you for using our plugin. We would really appreciate if you could take a moment to drop a quick review that will inspire us to keep going.", 'product-reviews-import-export-for-woocommerce'), '<b>', '</b>');

            /* button texts */
            $this->later_btn_text   = __("Remind me later", 'product-reviews-import-export-for-woocommerce');
            $this->never_btn_text   = __("Not interested", 'product-reviews-import-export-for-woocommerce');
            $this->review_btn_text  = __("Review now", 'product-reviews-import-export-for-woocommerce');
...
?>
        <div class="<?php echo $this->banner_css_class; ?> notice-info notice is-dismissible">
            ...
            <div class="wt-cli-review-footer" style="position: relative;">
                <span class="wt-cli-footer-icon" style="position: absolute;right: 0;bottom: 10px;"><img src="<?php echo plugins_url(basename(plugin_dir_path(WF_PrRevImpExpCsv_FILE))).$this->webtoffee_logo_url; ?>" style="max-width:100px;"></span>
            </div>
        </div>
    <?php
    }
...
new Product_review_import_export_Review_Request();
